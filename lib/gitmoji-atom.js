...
module.exports = {
    activate(state) {
        // this.gitmojiFilter = new ArrayFilterer();
        // this.gitmojiFilter.setCandidates(Gitmojis.gitmojis, "description");
        this.gitmojiAtomView = new SelectList({
            ...
            //     });
            // },
            filterKeyForItem: gitmoji => {
                // If not using zadeh
                return gitmoji.description + " " + gitmoji.code;
            },
            didCancelSelection: () => {
    ...
    confirm(item) {
        shouldShowNotifications = !atom.config.get(
            "gitmoji-atom.noNotifications"
        );
        ...
        if (item !== "") {
            valueToSubmit = item[atom.config.get("gitmoji-atom.outputType")];
            let editor = atom.workspace.getActiveTextEditor();
            switch (atom.config.get("gitmoji-atom.submitMode")) {
                case "git":
                    let commitEditor = document.querySelector(
                        ".github-AtomTextEditor-container > atom-text-editor"
                    );
                    if (commitEditor !== null && commitEditor !== undefined) {
